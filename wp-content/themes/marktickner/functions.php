...
// Return a list of categories like the 'the_category()' method, but with some excluded
function the_category_filter( $thelist, $separator=' ' ) {
	if(!defined( 'WP_ADMIN' )) {
		// List the category names to exclude
		$exclude = array( 'My Work' , 'Something Else' );
		$cats = explode( $separator, $thelist );
		$newlist = array();
		foreach( $cats as $cat ) {
			$cat_name = trim( strip_tags( $cat ) );
			if( !in_array( $cat_name, $exclude ) )
				$newlist[] = $cat;
		}
