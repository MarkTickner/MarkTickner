...
class wpbgallery {
    ...
    private function thumb($id, $post_id) {
        ...
        return PHP_EOL . '<li><a href="' . $url . '" title="' . $title . '" data-gallery=""><img src="' . $image[0] . '" width="' . $image[1] . '" height="' . $image[2] . '" alt="' . $info[0]->post_title . '" /></a></li> '; // EDITED (Added space after </li>
    }
    ...
    private function gallery($post_id = false) {
        ...
        if (is_array($gallery) && count($gallery) > 0) {
            $html = '<div id="wpbgallery_container">' . PHP_EOL . '<ul id="wpbgallery" class="clearfix">';
            foreach ($gallery as $thumbid) {
                $html .= $this->thumb($thumbid, $post_id);
            }
            $html .= PHP_EOL . ' <span class="stretch"></span> ' . PHP_EOL . ' </ul> ' . PHP_EOL . ' </div> ' . PHP_EOL; // EDITED (Added end of lines PHP_EOL and span)
        }
